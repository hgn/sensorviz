...
def draw(ax, daily, monthly, date_column, data_column, average_temp, title, unit, color):
    ...
    ax.scatter(max_temp_date, max_temp_value, color='red', label=f'Max {title}', marker='o', s=20)
    ax.annotate(f'Max: {max_temp_value:.1f}{unit}', (max_temp_date, max_temp_value), fontsize=8, textcoords="offset points", xytext=(10, 0), ha='left', bbox=dict(boxstyle='round, pad=0.4', edgecolor='none', facecolor='white'))

    ax.scatter(min_temp_date, min_temp_value, color='green', label=f'Min {title}', marker='o', s=20)
    ax.annotate(f'Min: {min_temp_value:.1f}{unit}', (min_temp_date, min_temp_value), fontsize=8, textcoords="offset points", xytext=(10, 0), ha='left', bbox=dict(boxstyle='round, pad=0.4', edgecolor='none', facecolor='white'))
    ...
    for date, temp in zip(monthly.index, monthly.values):
        ...
        ax.annotate(disp, (date, temp), fontsize=8, textcoords="offset points", xytext=(0, 10), ha='center', bbox=dict(boxstyle='round, pad=0.4', edgecolor='none', facecolor='white'))
...
def visualize_temp(df, ax1):
    ...
    draw(ax1, daily, monthly, df[date_column], df[temperature_column], average_temp, "Temperature", "°C", "blue")
...
def visualize_hum(df, ax1):
    ...
    average_temp = np.mean(df[temperature_column])
    draw_year_separator(ax1, df[date_column])
    draw(ax1, daily, monthly, df[date_column], df[temperature_column], average_temp, "Humanity", "%", "magenta")
...
def generate(filename):
    ...
    fig, (ax1, ax2) = plt.subplots(figsize=(12, 12), nrows=2, ncols=1, sharex=True)  # Set the figure size
